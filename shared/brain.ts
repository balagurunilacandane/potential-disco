...
// Runs anywhere: the server persists it to disk, the browser demo keeps it in memory.
import {
  AGENT_STATUSES,
  MAX_AGENTS_PER_TEAM,
  type Agent,
...
import { TEAM_COLORS, TEAM_ICONS, fillTemplate, pick, randomAppearance, roleTemplate } from './catalog.ts';
...
export type BrainListener = (event: ServerEvent) => void;

export class BrainCore {
  protected data: BrainData;
  private listeners = new Set<BrainListener>();

  constructor(data?: BrainData | null) {
    this.data = data ?? seedBrain();
  }
  ...
  /** Called after every change — the server overrides it to persist. */
  protected changed() {}
...
export function seedBrain(): BrainData {
  const now = Date.now();
